...
├── agent-tools/
│   ├── index.html        Agent Tools — the 11 AI agents (password-gated)
...
Open `/agent-tools/`. Default password: **`SCI-Agent-2026!`** — change it.
...
Paste the result into `PASSWORD_HASH` near the top of the script in `agent-tools/index.html`.
...
### Editing the agents
Open `agent-tools/agents.js` in any text editor. The prompts are plain English. Rewrite them as the
