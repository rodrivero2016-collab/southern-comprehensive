...
(function () {
  ...
  function render() {
    ...
    host.innerHTML =
      ...
      '<div class="panel panel--warn" style="margin-top:22px">' +
        '<h4>This lives in this browser only</h4>' +
        '<p class="mb0">Not synced, not backed up, not visible on your phone. If you clear browsing data it is ' +
        'gone. <strong>Export to CSV weekly.</strong> By design this holds no phone numbers, emails, dates of ' +
        'birth, medications or Medicare numbers &mdash; keep those in your carrier systems.</p>' +
      '</div>';
  ...
  function form(id) {
    ...
    wrap.querySelector('#f-save').onclick = function () {
      ...
      if (r) rows = rows.map(function (x) { return x.id === r.id ? rec : x; });
      else rows.unshift(rec);
      save(rows); wrap.remove(); render();
    };
  ...
  window.SCI_pipeline = { render: render };
})();
